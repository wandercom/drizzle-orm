...
await Promise.all([
	(async () => {
		await $`pnpm exec tsup`.stdio('pipe', 'pipe', 'pipe');
	})(),
	(async () => {
		await $`pnpm exec tsc -p tsconfig.dts.json`.stdio('pipe', 'pipe', 'pipe');
...
await Promise.all([
	$`pnpm exec tsup src/version.ts --no-config --dts --format esm --outDir dist.new`.stdio('pipe', 'pipe', 'pipe'),
	$`pnpm exec tsup src/version.ts --no-config --dts --format cjs --outDir dist.new`.stdio('pipe', 'pipe', 'pipe'),
]);
